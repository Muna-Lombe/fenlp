...
const service = {
  ...
  pdfDocuments: () => FIND_MANY_RESULT,
  pdfDocument: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("PdfDocument", () => {
  ...
  test("GET /pdfDocuments/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/pdfDocuments"}/${nonExistingId}`)
  ...
  test("GET /pdfDocuments/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/pdfDocuments"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /pdfDocuments existing resource", async () => {
